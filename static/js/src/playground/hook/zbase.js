...
export class Hook extends AcGameObject {
  ...
  add_money() {
    ...
    console.log(
      ...
      this.catched_money,
      "all:",
      this.score_number.shop_money_number
    );
  }
  ...
  update_tile_length() {
    ...
    if (
      this.direction_flag === 3 &&
      (this.catched ||
        Math.abs(this.max_tile_length - this.tile_length) < this.eps)
    ) {
    ...
    if (
      this.direction_flag === 4 &&
      Math.abs(this.tile_length - this.min_tile_length) < this.eps
    ) {
  ...
  load_image() {
    ...
    this.images = [this.hook_sheet1, this.hook_sheet0, this.ropetile];
  }
  ...
  draw_tile(canvas, scale, angle, num) {
    ...
    for (let i = 1; i < num; i++) {
      this.ctx.drawImage(
        ...
        0,
        this.ropetile.width,
        this.ropetile.height,
        -this.ropetile.height *
          Math.sin((20 * Math.PI) / 180) *
          canvas.scale *
          i,
        (this.ropetile.height / Math.cos((20 * Math.PI) / 180)) *
          canvas.scale *
          i,
